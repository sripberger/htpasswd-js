...
class Htpasswd {
	...
	/**
	 * Similar to `#authenticate`, but will force bcrypt hashing to occur in a
	 * single event loop, so that the result can be returned directly instead of
	 * in a promise.
	 * @param {String} username - Username which may or may not exist.
	 * @param {password} password - Password which may or may not be correct.
	 * @returns {Promise<boolean>} - true if the combination is valid, false
	 *   otherwise.
	 */
